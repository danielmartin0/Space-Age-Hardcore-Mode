...
script.on_event(defines.events.on_gui_click, function(event)
	...
	if
		...
		and not remote.call(abort_interface_name, "should_abort", character)
	then
...
script.on_event(defines.events.on_tick, function(event)
	...
	for _, player in pairs(game.players) do
		if player and player.valid then
			...
			if entity and entity.valid and is_silo(entity) and entity.surface and entity.surface.valid then
				local relative = player.gui.relative
				if relative[GUI_KEY] then
					update_gui_state(player, entity, entity.surface.planet)
				end
