...
Public.token = Global.register({ data = data, settings = settings }, function(tbl)
	data = tbl.data
...
function Public.uid_name()
	...
	local filepath = info.source:match("^@__level__/(.+)$") or info.source:match("^@__pirate_ship__/(.+)$")
...
function Public.add_main_frame_with_toolbar(
	player,
	align,
	set_frame_name,
	set_settings_button_name,
...
local function handler_factory(event_id)
	...
	return function(element_name, handler)
		...
		if not handler or not type(handler) == "function" then
			return error(
				"Handler is required when passing it onto the handler_factory and needs to be of type function.",
				2
			)
		end
...
function Public.add_tab_to_gui(tbl)
	...
	if not main_gui_tabs[tbl.caption] then
		main_gui_tabs[tbl.caption] =
			{ id = tbl.id, name = tbl.name, admin = admin, only_server_sided = only_server_sided }
	else
...
local function top_button(player)
	if settings.mod_gui_top_frame then
		local button = Public.add_mod_button(
			player,
...
local function top_toggle_button(player)
	...
	if Public.get_mod_gui_top_frame() then
		local b = Public.add_mod_button(player, {
			type = "sprite-button",
...
local function draw_main_frame(player)
	...
	local frame, inside_frame =
		Public.add_main_frame_with_toolbar(player, "left", main_frame_name, nil, close_button_name, "Comfy Factorio")
...
Public.on_click(main_button_name, function(event)
	...
	if frame then
		...
		Event.raise(
			Public.events.on_gui_closed_main_frame,
...
Public.on_click(close_button_name, function(event)
	local player = event.player
	local frame = Public.get_parent_frame(player)
	local active_frame = Public.get_player_active_frame(player)
...
Public.on_custom_close(main_frame_name, function(event)
	local player = event.player
	local active_frame = Public.get_player_active_frame(player)
	Event.raise(Public.events.on_gui_closed_main_frame, { player_index = player.index, element = active_frame or nil })
	local frame = Public.get_parent_frame(player)
...
Public.on_click(main_toggle_button_name, function(event)
